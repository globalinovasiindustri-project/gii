import { pgTable, index, foreignKey, uuid, timestamp, text, boolean, unique, integer, check } from "drizzle-orm/pg-core"
...
export const carts = pgTable("carts", {
	...
	index("cart_session_id_idx").using("btree", table.sessionId.asc().nullsLast().op("text_ops")).where(sql`(session_id IS NOT NULL)`),
	...
	check("carts_identifier_check", sql`(user_id IS NOT NULL) OR (session_id IS NOT NULL)`),
]);
